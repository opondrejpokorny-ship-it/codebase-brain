...
function MissingContextQueueAudit({ metadata }) {
...
export default function ImportMetadataCard({ project }) {
  ...
  const hasWarnings = errors.length > 0 || metadata.truncatedTree || hasFocusedResolveWarnings || hasPersistentQueueWarnings;

  return (
    <div className="bg-white rounded-xl border border-slate-200 p-5">
      <div className="flex items-start justify-between gap-3 mb-4">
        <div><h3 className="font-heading font-semibold text-sm text-slate-900 flex items-center gap-2"><GitBranch className="w-4 h-4 text-slate-500" />Import Details</h3><p className="text-xs text-slate-400 mt-1">Small audit trail for this repository import.</p></div>
        <span className={`inline-flex items-center gap-1 text-xs px-2 py-1 rounded-md ${hasWarnings ? "bg-amber-50 text-amber-700" : "bg-emerald-50 text-emerald-700"}`}>{hasWarnings ? <AlertTriangle className="w-3 h-3" /> : <CheckCircle2 className="w-3 h-3" />}{hasWarnings ? "Warnings" : "OK"}</span>
      </div>
      ...
      {errors.length > 0 && <div className="mt-3 border-t border-slate-100 pt-3"><p className="text-xs font-medium text-slate-500 mb-2">File import warnings</p><div className="space-y-1 max-h-24 overflow-y-auto">{errors.slice(0, 8).map((error, index) => <p key={`${error.path || "error"}-${index}`} className="text-xs text-amber-700 font-mono truncate">{error.path || "unknown"}: {error.message || "failed"}</p>)}{errors.length > 8 && <p className="text-xs text-slate-400">+{errors.length - 8} more warnings</p>}</div></div>}
    </div>
  );
}
