...
function MissingContextQueueCard({ project, projectId, queue = [], files = [], resolving = false, resolveMessage = "", resolveError = "", onResolve, onClear }) {
        ...
        {resolvedQueue.map((item) => (
          <div key={item.target} className="bg-white/60 rounded-md px-2 py-1.5">
            <div className="flex items-start justify-between gap-2">
              ...
              <Badge variant="outline" className={`${queueStatusBadgeClass(item.status)} flex-shrink-0 text-[10px] px-1.5 py-0`}>{item.status === "indexed" ? "Indexed" : "Missing"}</Badge>
            </div>
